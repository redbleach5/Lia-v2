  ...
  try {
    ...
    const text = await Promise.race([
      ...
      new Promise<string>((_, reject) =>
        setTimeout(() => reject(new Error(`plan generation timeout (${LLM_TIMEOUT_MS/1000}s)`)), LLM_TIMEOUT_MS)
      ),
    ]);
  ...
  if (tryWithTools) {
    ...
    try {
      ...
      fullText = await Promise.race([
        result.text,
        new Promise<string>((_, reject) =>
          setTimeout(() => reject(new Error(`streamText timeout (${LLM_TIMEOUT_MS/1000}s)`)), LLM_TIMEOUT_MS)
        ),
      ]);
...
${stepsBlock}`;
  ...
  try {
    ...
    const text = await Promise.race([
      result.text,
      new Promise<string>((_, reject) =>
        setTimeout(() => reject(new Error(`synthesize timeout (${SYNTHESIS_TIMEOUT_MS/1000}s)`)), SYNTHESIS_TIMEOUT_MS)
      ),
    ]);
