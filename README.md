...
По умолчанию 3D-режим пытается загрузить `/models/Lia.vrm`. Этого файла нет
в репозитории (бинарники не коммитятся) — поэтому при первом запуске вы
увидите в консоли:

```
GET /models/Lia.vrm 404
[VRM] load failed: HttpError: fetch for ".../Lia.vrm" responded with 404
[AvatarColumn] VRM failed, falling back to Live2D
```

Это **нормально** — приложение автоматически переключается на Live2D-стилизованный
аватар. Чтобы вернуть 3D, в настройках приложения: **Настройки → Внешний вид →
Скачать готовую** (одной кнопкой — скачает sample.vrm из репозитория Pixiv)
или **Загрузить свой .vrm**.

Если и Live2D падает с ошибкой `checkMaxIfStatementsInShader` (некоторые GPU/
драйверы), приложение дополнительно переключается на простой SVG-фоллбэк.
