...
const ASK_USER_TIMEOUT_MS = 10 * 60 * 1000; // 10 min timeout for user response (increased from 5min)
...
const LLM_TIMEOUT_MS = parseInt(process.env.LIA_LLM_TIMEOUT_MS || '180000', 10); // 3 минуты дефолт
const SYNTHESIS_TIMEOUT_MS = parseInt(process.env.LIA_LLM_SYNTHESIS_TIMEOUT_MS || String(Math.max(LLM_TIMEOUT_MS, 240000)), 10); // 4 минуты дефолт
...
export async function sweepStaleTasks(): Promise<number> {
  try {
    ...
    const resumable = staleTasks.filter(t => t.status === 'executing' && t.checkpointJson);
    const failed = staleTasks.filter(t => !(t.status === 'executing' && t.checkpointJson));
    ...
    logger.warn('agent', `Swept ${staleTasks.length} stale task(s)`, {
...
function computeWallTimeMs(maxDurationSec: number | null): number {
  if (!maxDurationSec) return MIN_WALL_TIME_MS;
...
export async function runAgentTask(taskId: string): Promise<void> {
  ...
  const watchdogTimer = setTimeout(() => {
    if (activeRunners.has(taskId)) {
      ...
      abortTask(taskId);  // прерывает активный streamText
  ...
  try {
    ...
    if (task.checkpointJson) {
      // RESUME — пропускаем PLAN, восстанавливаем состояние
      try {
        const checkpoint = JSON.parse(task.checkpointJson) as Checkpoint;
        ...
        // Emit plan_ready для UI (клиент мог переподключиться)
        emitAgentEvent({
        ...
        // Emit уже выполненные шаги для UI (replay)
        ...
        // Чистим битый checkpoint, продолжаем с PLAN
        await updateAgentTask(taskId, { checkpointJson: null, stepsJson: '[]', currentStep: 0 });
        plan = await generatePlan(task, toolDescriptions);
        steps = [];
      }
    } else {
    ...
    for (let i = startStep; i < task.maxSteps; i++) {
      ...
      await updateAgentTask(taskId, {
        currentStep: i + 1,
        stepsJson: JSON.stringify(steps),
        // Phase 4.1: checkpoint для resume после restart.
        // Сохраняем plan + steps + timestamp — runner при resume пропустит PLAN.
        checkpointJson: JSON.stringify({ plan, steps, savedAt: Date.now() }),
      });
    ...
    await updateAgentTask(taskId, {
      ...
      resultSummary,
      stepsJson: JSON.stringify(steps),
      checkpointJson: null,  // Phase 4.1: очищаем checkpoint при завершении
    });
...
Верни СТРОГО JSON.`;
  ...
  try {
    ...
    // AbortSignal.timeout: в отличие от Promise.race + setTimeout,
    // таймер автоматически очищается и streamText получает abort.
...
${stepsBlock}`;
  ...
  try {
    ...
    // Таймаут SYNTHESIS_TIMEOUT_MS — синтез может быть длинным на сложных задачах.
    const text = await result.text;
...
export async function cancelAgentTaskRun(taskId: string): Promise<void> {
  ...
  signalCancellation(taskId);
  cancelWaiting(taskId);
  abortTask(taskId);  // прерывает активный streamText — LLM-вызов завершается cleanly
